...
func (c *uploadClient) UploadMedia(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[UploadMediaRequest, UploadMediaResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Upload_ServiceDesc.Streams[0], Upload_UploadMedia_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type Upload_UploadMediaClient = grpc.BidiStreamingClient[UploadMediaRequest, UploadMediaResponse]
...
func (UnimplementedUploadServer) UploadMedia(grpc.BidiStreamingServer[UploadMediaRequest, UploadMediaResponse]) error {
...
func _Upload_UploadMedia_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(UploadServer).UploadMedia(&grpc.GenericServerStream[UploadMediaRequest, UploadMediaResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Upload_UploadMediaServer = grpc.BidiStreamingServer[UploadMediaRequest, UploadMediaResponse]
