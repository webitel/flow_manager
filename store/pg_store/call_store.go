...
func (s SqlCallStore) SaveMediaStats(stats *model.CallActionMediaStats) *model.AppError {
  ...
  on conflict (sip_id, domain_id) do update set
          created_at        = now(),
          user_id           = excluded.user_id,
          mos_avg           = excluded.mos_avg,
          mos_min           = excluded.mos_min,
          mos_min_at        = excluded.mos_min_at,
          mos_max           = excluded.mos_max,
          mos_max_at        = excluded.mos_max_at,
          jitter_avg        = excluded.jitter_avg,
          jitter_min        = excluded.jitter_min,
          jitter_min_at     = excluded.jitter_min_at,
          jitter_max        = excluded.jitter_max,
          jitter_max_at     = excluded.jitter_max_at,
          packetloss_avg    = excluded.packetloss_avg,
          packetloss_min    = excluded.packetloss_min,
          packetloss_min_at = excluded.packetloss_min_at,
          packetloss_max    = excluded.packetloss_max,
          packetloss_max_at = excluded.packetloss_max_at,
          roundtrip_avg     = excluded.roundtrip_avg,
          roundtrip_max     = excluded.roundtrip_max,
          roundtrip_max_at  = excluded.roundtrip_max_at,
          roundtrip_min     = excluded.roundtrip_min,
          roundtrip_min_at  = excluded.roundtrip_min_at`, map[string]any{
		...
		"JitterAvg":       stats.RTP.Jitter.Average,
		"JitterMin":       stats.RTP.Jitter.Min,
		"JitterMinAt":     stats.RTP.Jitter.MinAt,
		"JitterMax":       stats.RTP.Jitter.Max,
		"JitterMaxAt":     stats.RTP.Jitter.MaxAt,
