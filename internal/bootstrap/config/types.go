...
const (
	AppServiceName    = "workflow"
	AppServiceTTL     = time.Second * 30
	AppDeregisterCriticalTTL = time.Second * 60

	HeaderFromServiceName = "From-Service"
	SchemaCacheSize       = 10000
	SchemaCacheExpire     = (60 * 60) * 24 // 24 hours
...
type ChatTemplatesSettings struct {
	Path string `json:"host" flag:"chat_templates_path|./message_templates|Path to the root folder of the chat message templates" env:"CHAT_TEMPLATES_PATCH"`
}
